...
class MECHARTS_API SMEWebBrowser : public SCompoundWidget
{
public:
	...
	SLATE_BEGIN_ARGS(SMEWebBrowser)
		: _FrameRate(60)//帧率
		, _InitialURL(TEXT("https://www.google.com"))//初始URL
		, _ShowErrorMessage(true)//是否显示错误信息
		, _SupportsTransparency(false)//是否支持透明度
		, _SupportsThumbMouseButtonNavigation(true)//是否支持拇指鼠标按钮导航
		, _ShowInitialThrobber(true)//是否在浏览器初始化期间显示抖动覆盖
		, _BackgroundColor(255, 255, 255, 255)
		, _EnableMouseTransparency(false)
		, _EnableVirtualPointerTransparency(false)
		, _TransparencyDelay(0.1f)
		, _TransparencyThreshold(0.333f)//透明度阈值
